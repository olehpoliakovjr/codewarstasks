...
function findShort(s) {
  ...
  let word = arrayOfWords
    .map((t) => ({ str: t, size: t.length }))
    .sort((a, b) => a.size - b.size)[0].str;
  return word.length;
}
...
function accum(s) {
  let array = s
    .split("")
    .map((c, b) => c.toUpperCase() + c.toLowerCase().repeat(b));
  return array;
}
...
function minMax(arr) {
  ...
  array.push(minNumber, maxNumber);
  return array;
}
minMax([2334454, 5, 3, 1, 232, 42, 13]);
